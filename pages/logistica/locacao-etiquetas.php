...
if ($debug) varDump2($_SESSION['LOCACAO']); 

if (is_array($_SESSION['LOCACAO']) && !is_null($_SESSION['LOCACAO'])) {
	$ETIQUETAS = array();
	for ($j=1; $j < $_GET['POSICAOINI']; $j++) { 
		array_push($ETIQUETAS, null);
	}
	foreach ($_SESSION['LOCACAO'] as $keytmp => $valuetmp) {
		((!isset($valuetmp['QTETIQUETA'])) ? $valuetmp['QTETIQUETA'] = 1:'');
...
foreach ($PAGINA as $keyP => $valueP) {
	...
	foreach ($valueP as $ETIQ_ATUAL_PAG => $valueI) {
		...
		if (is_null($valueI)) {
			...
			$pdf->Cell(22, 6, mb_convert_encoding("LOCAL:  ", 'Windows-1252'), 0, 0, 'L');
			$pdf->SetFont($font_helvetica, $style_b, $tam_22);
			$pdf->Cell(40, 6, $valueI['LOCACAO'], 0, 1, 'R');

			$pdf->Code39(($pos_x +15),($pos_y +10),$valueI['LOCACAO'],1,15);
