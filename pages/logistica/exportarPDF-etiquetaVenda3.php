...
foreach ($PDF['ITENS'] as $keyP => $valueI) {
	...
	$pdf->Cell(25, 5, ($valueI['LOCACAO'])?trim($valueI['LOCACAO']):'', 0, 1, 'L');

	if ($PDF['CABECALHO']['EXIBIRCODPECAETIQUETA'] == 'S') {
		$pdf->Cell(60, 5, "", 0, 0);
		$pdf->SetFont('arial','','10');
		$pdf->Cell(15, 5, "PEÇA: ", 0, 0, 'R');
		$pdf->SetFont('arial','B','10');
		$pdf->Cell(25, 5, $valueI['CODPECA'], 0, 1, 'L');
	} else {
		$pdf->Cell(60, 5, "", 0, 0);
		$pdf->SetFont('arial','','10');
		$pdf->Cell(15, 5, "QTD: ", 0, 0, 'R');
		$pdf->SetFont('arial','B','10');
		$pdf->Cell(25, 5, intval($valueI['QT']), 0, 1, 'L');
	}
	...
	$pdf->SetFont('arial','B','10');
	$pdf->Cell(25, 5, formataDataOracletoBr($PDF['CABECALHO']['DATA']), 0, 1, 'L');
	...
	$pdf->SetFont('arial','B','10');
	$pdf->Cell(25, 5, trim($valueI['QT']), 0, 1, 'L');
	...
	$pdf->SetFont('arial','B','10');
	$pdf->Cell(25, 6, $PDF['CABECALHO']['NUMPED'], 0, 1, 'L');


	$pdf->Cell(15, 15, "", 0, 0);
	// $pdf->Code39(($pos_x +12),($pos_y +9),reset(explode('-', str_replace('W', '', $valueI['CODPROD']))), 1.2, 7);
	...
	$pdf->Cell(85, 5, substr($valueI['PRODUTO'],0,40), 0, 1, 'C');

	$pdf->Cell(13, 5, "", 0, 0);
	$pdf->SetFont('arial','','8');
	$pdf->Cell(50, 5, 'NUMORIGINAL:', 0, 0, 'R');
	$pdf->SetFont('arial','b','12');
	$pdf->Cell(35, 5, (is_null($valueI['NUMORIGINAL'])?'':substr($valueI['NUMORIGINAL'],0,40)), 0, 1, 'L');

	$pdf->Cell(13, 5, "", 0, 0);
	$pdf->SetFont('arial','','8');
	$pdf->Cell(50, 5, 'LOCAL:', 0, 0, 'R');
	$pdf->SetFont('arial','b','12');
	$pdf->Cell(35, 5, substr(($valueI['LOCACAO'])?trim($valueI['LOCACAO']):'',0,40), 0, 1, 'L');

	$pdf->Cell(13, 5, "", 0, 0);
	$pdf->SetFont('arial','','8');
	$pdf->Cell(50, 5, 'QTD PEDIDA:', 0, 0, 'R');
	$pdf->SetFont('arial','b','12');
	$pdf->Cell(35, 5, substr(($valueI['QT'])?trim($valueI['QT']):'0',0,40), 0, 1, 'L');

	$pdf->Cell(13, 5, "", 0, 0);
	$pdf->SetFont('arial','','8');
	$pdf->Cell(50, 5, 'QTD ESTOQUE:', 0, 0, 'R');
	$pdf->SetFont('arial','b','12');
	$pdf->Cell(35, 5, substr(($valueI['QT'])?trim($valueI['QT']):'0',0,40), 0, 1, 'L');
