...
if (!$PAGINA) {
	...
	$pdf->SetTextColor($r_black,$g_black,$b_black);
	$pdf->Cell(30, $alturaLinha, (!is_null($extrato[0]['NUMORIGINAL']))?$extrato[0]['NUMORIGINAL']:"" , 'B', 0, 'C', 1);
	$pdf->Cell(30, $alturaLinha, 'W'.$extrato[0]['CODPROD'].' - '.$extrato[0]['DV'] , 'B', 0, 'C', 1);
	$pdf->Cell(125, $alturaLinha, (!is_null($extrato[0]['DESCRICAO']))?$extrato[0]['DESCRICAO']:"" , 'B', 0, 'C', 1);
	$pdf->Cell(60, $alturaLinha, (!is_null($extrato[0]['MARCA']))?$extrato[0]['MARCA']:"" , 'B', 0, 'C', 1);
	$pdf->Cell(30, $alturaLinha, (!is_null($extrato[0]['LOCACAO']))?$extrato[0]['LOCACAO']:"" , 'B', 1, 'C', 1);
	

	$pdf->Cell(275, $alturaLinha, "" , 0, 1);
	...
	foreach ($PAGINA as $key1 => $paginaAtual) {
		...
		$pendentes = buscaPedidosPendentesCODPROD($extrato[0]['CODPROD']);
		if ($pendentes) {

			//MONTA O CABEÇALHO DO ORCAMENTO
			$pdf->SetFont($font_arial,$style_b,$tam_12);
			$pdf->SetTextColor($r_black,$g_black,$b_black);
			$pdf->Cell(235, $alturaLinha, "ESTOQUE RESERVADO" , 'B', 1, 'L');
		...
		$pdf->SetFont($font_arial,$style_b,$tam_12);
		$pdf->SetTextColor($r_black,$g_black,$b_black);
		$pdf->Cell(275, $alturaLinha, converterUTF8("HISTÓRICO DE MOVIMENTAÇÃO") , 'B', 1, 'L');
