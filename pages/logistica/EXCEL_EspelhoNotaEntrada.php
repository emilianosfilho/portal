...
if (!empty($_GET['NUMNOTA'])) {

	if( $CAB = buscaCabecalhoNFEntrada($_GET)){
		if($debug) varDump2($CAB);	

		if ($ITENS = buscaProdutosNF($_GET)) {
			if($debug) varDump2($ITENS);	
		}
		else {
			exibeMensagem("Nenhum item encontrado para a NUMNOTA: {$_GET['NUMNOTA']}");
			fechaAba();
		}
	}
	else {
		exibeMensagem("Nenhum cabeçalho encontrado para a NUMNOTA: {$_GET['NUMNOTA']}");
		fechaAba();
	}

} 
else {
	exibeMensagem("NUMNOTA não informado");
	fechaAba();
}
...
header ("Content-Disposition: attachment; filename=\"ESPELHO_NF_ENT_{$_GET['NUMNOTA']}.xls\"" );
