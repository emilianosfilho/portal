...
$INVENTARIO = buscaDadosInventario($_GET);
// varDump2($INVENTARIO);

//TÍTULO DO RELATÓRIO
$titulo = "ESPELHO DO INVENTÁRIO ";

//ENDEREÇO ONDE SERÁ GERADO O PDF
$end_final = $INVENTARIO['IDINVENTARIO'].'_'.str_replace(" ", "_", $titulo).".pdf";
...
$regTotal = count($INVENTARIO['ITEM']);
...
$contPag=0;
$contReg=0;
foreach ($INVENTARIO['ITEM'] as $key => $value) {
...
foreach ($ARRAY_EXIBE as $paginaAtual => $pagina) {
	...
	$pdf->Cell(30, 8, "DATA" , 'B', 0, 'L', 1);
	$pdf->Cell(80, 8, "CONFERENTE" , 'B', 0, 'C', 1);
	$pdf->Cell(40, 8, "STATUS" , 'B', 0, 'C', 1);
	$pdf->Cell(40, 8, "LOCAÇÃO" , 'B', 1, 'C', 1);


	$pdf->SetFont($font_helvetica,$style_n,$tam_9);
	$pdf->SetFillColor($r_white,$g_white,$b_white);
	$pdf->SetTextColor($r_black,$g_black,$b_black);
	$pdf->Cell(30, 8, formataDataOracletoBr($INVENTARIO['DATA']) , 'B', 0, 'L');
	$pdf->Cell(80, 8, $INVENTARIO['CONFERENTE'] , 'B', 0, 'C');
	$pdf->Cell(40, 8, $INVENTARIO['STATUSINVENTARIO'] , 'B', 0, 'C');
	$pdf->Cell(40, 8, $INVENTARIO['LOCACAO'] , 'B', 1, 'C');
	...
	$pdf->SetTextColor($r_black,$g_black,$b_black);
	$pdf->SetFillColor($r_light,$g_light,$b_light);
	$pdf->Cell(10, 8, "#"			  		, 0, 0, 'L', 1);
	$pdf->Cell(25, 8, "CODPROD"		  		, 'L', 0, 'L', 1);
	$pdf->Cell(35, 8, "NUMORIGINAL"		  	, 'L', 0, 'L', 1);
	$pdf->Cell(60, 8, "DESCRIÇÃO"		  	, 'L', 0, 'L', 1);
	$pdf->Cell(35, 8, "MARCA"		  		, 'L', 0, 'L', 1);
	$pdf->Cell(20, 8, "Qt. ESTOQUE"		  	, 'L', 0, 'C', 1);
	$pdf->Cell(20, 8, "Qt. PED"		  		, 'L', 0, 'C', 1);
	$pdf->Cell(20, 8, "Qt. AVAR"		  	, 'L', 0, 'C', 1);
	$pdf->Cell(20, 8, "Qt. CONF"			, 'L', 0, 'C', 1);
	$pdf->Cell(35, 8, "CONFERENTE"			, 'L', 1, 'C', 1);
	...
	foreach ($pagina as $itemAtual => $item) {
		...
		$pdf->SetFillColor($r_white,$g_white,$b_white);
		$pdf->SetTextColor($r_black,$g_black,$b_black);
		$pdf->Cell(10, 8, $contadorGeral 						, 0, 0, 'L');
		$pdf->Cell(25, 8, 'W'.$item['CODPROD'].'-'.$item['DV'] 	, 'L', 0, 'L');
		$pdf->Cell(35, 8, $item['NUMORIGINAL'] 					, 'L', 0, 'L');
		$pdf->Cell(60, 8, $item['DESCRICAO'] 					, 'L', 0, 'L');
		$pdf->Cell(35, 8, $item['MARCA'] 						, 'L', 0, 'L');
		$pdf->Cell(20, 8, $item['QTESTOQUE']					, 'L', 0, 'C');
		$pdf->Cell(20, 8, $item['QTPEDIDO'] 					, 'L', 0, 'C');
		$pdf->Cell(20, 8, $item['QTAVARIA'] 					, 'L', 0, 'C');
		$pdf->Cell(20, 8, (($item['QTCONFERIDA']=="")?'---':$item['QTCONFERIDA'])	, 'L', 0, 'C');
		$pdf->Cell(35, 8, $item['CONFERENTE'] 					, 'L', 1, 'L');
...
// 	$pdf->SetTextColor($r_black,$g_black,$b_black);
// 	$pdf->Cell(190, 4, "AV. MAX TEIXEIRA, Nº 1057, COLÔNIA SANTO ANTÔNIO, MANAUS/AM, CEP 69093-770", 0, 1, 'C');
// 	$pdf->SetFont($font_arial, $style_i, $tam_9);
// 	$pdf->Cell(190, 4, "Página ".$paginaAtual."/".$totalPaginas, 0, 1, 'C');
// }
