...
// 	/*##################################################################################################*/
	...
	$pdf->SetTextColor($r_success,$g_success,$b_success);
	$pdf->Cell(280, $alturaLinha,  "ITENS V�LIDOS NO PEDIDO DE VENDA"  , 0, 1); // Pula 1 linha

	$pdf->SetFont('arial','b',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(5, $alturaLinha, "#"					 	, 'B', 0);
	$pdf->Cell(18, $alturaLinha, "CodProd"				, 'B', 0);
...
// 	/*##################################################################################################*/
	...
	for ($l=$reginicio; $l <= $regfim; $l++) { 
		...
		$pdf->Cell(18, $alturaLinha, $PDF['ITENS'][$l]['CODPROD'].'-'.$PDF['ITENS'][$l]['DV']			, 0, 0);
...
// 	/*#####################################################################################################*/
	...
	// 	/*##################################################################################################*/
	if ($cortes = buscaItensCortados($NUMPEDRCA)) {
		
		$pdf->Cell(280, $alturaLinha,  ""  , 0, 1); // Pula 1 linha
		$pdf->SetFont('arial','b',12);
		$pdf->SetTextColor(139,0,0);
		$pdf->Cell(280, $alturaLinha,  "ITENS CORTADOS DO PEDIDO"  , 0, 1); // Pula 1 linha

		$pdf->SetFont('arial','b',8);
		$pdf->SetTextColor(0,0,0);
		$pdf->Cell(5, $alturaLinha, "#"						, 'B', 0);
		$pdf->Cell(18, $alturaLinha, "Cod Prod"				, 'B', 0);
		$pdf->Cell(42, $alturaLinha, "Descri��o"			, 'B', 0);
		$pdf->Cell(20, $alturaLinha, "Marca"				, 'B', 0);
		$pdf->Cell(20, $alturaLinha, "NumOriginal"			, 'B', 0);
		$pdf->Cell(13, $alturaLinha, "Qt.(Un)"				, 'B', 0);
		$pdf->Cell(20, $alturaLinha, "P.Venda"				, 'B', 0);
		$pdf->Cell(20, $alturaLinha, "Vl. Total"			, 'B', 0);
		$pdf->Cell(121, $alturaLinha, "Motivo"				, 'B', 0);
		$pdf->Cell(1, $alturaLinha,  ""  , 0, 1); // Quebra linha

		foreach ($cortes as $key => $cortado) {
			$pdf->SetFont('arial','',7);
			$pdf->Cell(5, $alturaLinha, $regAtual 			, 0, 0);
			$pdf->Cell(18, $alturaLinha, $cortado['CODPROD'].'-'.$cortado['DV']			, 0, 0);
			$pdf->Cell(42, $alturaLinha, $cortado['DESCRICAO']			, 0, 0);
			$pdf->Cell(20, $alturaLinha, $cortado['MARCA']			, 0, 0);
			$pdf->Cell(20, $alturaLinha, $cortado['NUMORIGINAL']		, 0, 0);
			$pdf->Cell(13, $alturaLinha, $cortado['QT']		, 0, 0);
			$pdf->Cell(20, $alturaLinha, $cortado['PVENDA']			, 0, 0);
			$pdf->Cell(20, $alturaLinha, $cortado['VALORTOTAL']		, 0, 0);
			$pdf->Cell(20, $alturaLinha, converterUTF8($cortado['OBSERVACAO_PC'])			, 0, 0);
			$pdf->Cell(1, $alturaLinha,  ""  , 0, 1); // Quebra linha
		}
	}
	...
	// 	/*#####################################################################################################*/
		...
		$pdf->SetFont('arial','',9);
		$pdf->Cell(190, $alturaLinha, converterUTF8($PDF['CABECALHO']['EMITENTE'])  		, 0, 0);
		$pdf->SetFont('arial','b',9);
		$pdf->Cell(30, $alturaLinha, "Vl. Pedido:" 			, 0, 0, 'R');
		$pdf->SetFont('arial','',9);
		$pdf->Cell(30, $alturaLinha, "R$ ".moeda($VALORTOTALPEDIDO)  		, 0, 0);
		$pdf->Cell(1, $alturaLinha,  ""  , 0, 1); // Quebra linha
	...
	// 	/*#####################################################################################################*/
		...
		$pdf->SetFont('arial','',9);
		$pdf->Cell(45, $alturaLinha, $PDF['CABECALHO']['FRETEREDESPACHO']  		, 0, 0);
		$pdf->SetFont('arial','b',9);
		$pdf->Cell(30, $alturaLinha, "Vl. Frete:" 			, 0, 0, 'R');
		$pdf->SetFont('arial','',9);
		$pdf->Cell(30, $alturaLinha, "R$ ".moeda($PDF['CABECALHO']['VLFRETE'])  		, 0, 0);
		$pdf->Cell(1, $alturaLinha,  ""  , 0, 1); // Quebra linha
	...
	// 	/*#####################################################################################################*/
		...
		$pdf->SetFont('arial','',9);
		$pdf->Cell(185, $alturaLinha, $PDF['CABECALHO']['OBSERVACAO'].' [## '.converterUTF8($PDF['CABECALHO']['OBSENTREGA1']).' ##]'.' [## '.converterUTF8($PDF['CABECALHO']['OBSENTREGA2']).' ##]'  		, 0, 0);
		$pdf->SetFont('arial','b',9);
		$pdf->Cell(30, $alturaLinha, "Vl. Total:" 			, 0, 0, 'R');
		$pdf->SetFont('arial','',9);
		$pdf->Cell(30, $alturaLinha, "R$ ".moeda($VALORTOTALPEDIDO + $PDF['CABECALHO']['VLFRETE'])  		, 0, 0);
		$pdf->Cell(1, $alturaLinha,  ""  , 0, 1); // Quebra linha
	} else {
