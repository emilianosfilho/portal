...
foreach ($paginas as $numPagina => $itensPagina) {
	...
	$pdf->Cell(50,15,"", 0, 0);
	$pdf->Cell(170,15, mb_convert_encoding("CONFERÊNCIA E EXPEDIÇÃO", "ISO-8859-1"), 0, 0, 'C', 1);
	...
	$pdf->Cell(85, $alturaLinha, substr($cabecalho['CODCLI']."- ".mb_convert_encoding($cabecalho['CLIENTE'], "ISO-8859-1"), 0, 40) 	, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(10, $alturaLinha, "CNPJ:" 						, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(52, $alturaLinha, $cabecalho['CNPJ']  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(30, $alturaLinha, mb_convert_encoding("Número do Pedido:", "ISO-8859-1") 				, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(30, $alturaLinha, $cabecalho['NUMPED']  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(35, $alturaLinha, mb_convert_encoding("Número do Orçamento:", "ISO-8859-1") 			, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(20, $alturaLinha, $cabecalho['NUMPEDCLI'], 0, 1, "R");

// 	/*#####################################################################################################*/
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(15, $alturaLinha, mb_convert_encoding("Endereço:", "ISO-8859-1") 			, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(85, $alturaLinha, mb_convert_encoding($cabecalho['ENDERECO'], "ISO-8859-1")  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	...
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(60, $alturaLinha, mb_convert_encoding($cabecalho['BAIRRO'], "ISO-8859-1")  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
...
// 	/*#####################################################################################################*/
	...
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(15, $alturaLinha, mb_convert_encoding("Município:", "ISO-8859-1") 						, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
...
// 	/*#####################################################################################################*/
	...
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(16, $alturaLinha, mb_convert_encoding("Cobrança:", "ISO-8859-1") 						, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(46, $alturaLinha, substr($cabecalho['CODCOB']."- ".$cabecalho['COBRANCA'], 0, 40)  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(27, $alturaLinha, "Plano Pagamento:" 						, 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
	$pdf->Cell(43, $alturaLinha, $cabecalho['CODPLPAG']."- ".mb_convert_encoding($cabecalho['PLPAG'], "ISO-8859-1")  		, 0, 0);
	$pdf->SetFont($font_arial,$style_b,$tam_8);
	$pdf->Cell(14, $alturaLinha, mb_convert_encoding("Posição:", "ISO-8859-1"), 0, 0);
	$pdf->SetFont($font_arial,$style_n,$tam_8);
...
// 	/*#####################################################################################################*/
	if (isset($cabecalho['OBSERVACAO_PC']) && !empty($cabecalho['OBSERVACAO_PC'])) {
		$pdf->SetFont($font_arial,$style_b,$tam_8);
		$pdf->Cell(25, $alturaLinha, mb_convert_encoding("Motivo Rejeição:", "ISO-8859-1"), 0, 0);
	...
	foreach ($itensPagina as $pos => $item) {
		...
		if ($pos == 0) {
			...
			$pdf->Cell(90, $alturaLinha, mb_convert_encoding("Descrição", "ISO-8859-1")			, 'B', 0, 0, 1);
			$pdf->Cell(43, $alturaLinha, "Marca"				, 'B', 0, 0, 1);
			$pdf->Cell(30, $alturaLinha, mb_convert_encoding("Locação", "ISO-8859-1")				, 'B', 0, 0, 1);
			$pdf->Cell(10, $alturaLinha, "UN"					, 'B', 0, 0, 1);
			$pdf->Cell(20, $alturaLinha, "Qtd"					, 'B', 0, 'R', 1);
			$pdf->Cell(30, $alturaLinha, mb_convert_encoding("Preço Un", "ISO-8859-1")				, 'B', 0, 'R', 1);
	...
	if ($totalPaginas == ($numPagina+1)) {
		...
		$pdf->Cell(213, $alturaLinha, mb_convert_encoding($cabecalho['EMITENTE'], "ISO-8859-1")  		, 0, 0);
		$pdf->SetFont($font_arial,$style_b,$tam_8);
		...
		$pdf->SetFont($font_arial,$style_b,$tam_8);
		$pdf->Cell(30, $alturaLinha, mb_convert_encoding("Observações: ", "ISO-8859-1") 			, 0, 0);
		$pdf->SetFont($font_arial,$style_n,$tam_10);
		$pdf->Cell(200, $alturaLinha, mb_convert_encoding($cabecalho['OBSENTREGA1'], "ISO-8859-1").mb_convert_encoding($cabecalho['OBSENTREGA2'], "ISO-8859-1") , 1, 0);
		$pdf->SetFont($font_arial,$style_b,$tam_8);
		...
		$pdf->Cell(280, 30, "", 0, 1);
		$pdf->SetFont($font_arial,$style_n,$tam_8);
		$pdf->Cell(100, $alturaLinha, mb_convert_encoding("SEPARADOR: ".$cabecalho['SEPARADOR'], "ISO-8859-1"), 'T', 0, 'C');
		$pdf->Cell(80, $alturaLinha, '', 0, 0);
		$pdf->Cell(100, $alturaLinha, mb_convert_encoding("CONFERENTE", "ISO-8859-1"), 'T', 1, 'C');
	...
	$pdf->Cell(140, $alturaLinha, mb_convert_encoding("Página ".($numPagina+1)." de ".$totalPaginas, "ISO-8859-1"), 'T', 1, 'R');
}
